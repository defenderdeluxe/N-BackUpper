...
public class MainView extends JFrame {
    ...
    // shows the full copy progress (calculated by the byte size)
    private JProgressBar progressBarAll;
    // shows the copy progress of one file
    private JProgressBar progressBarCurrent;
    private JScrollPane scrollPaneDetails;
    private JScrollPane scrollPaneSource;
    // informs the user about the current task
    private JTextArea textAreaDetails;
    // holds the target drive
    private JTextField textFieldTarget;
    private MainController controller;

    public MainView(MainController controller) {
        ImageIcon img = new ImageIcon(getClass().getResource("nb.png"));
        this.setIconImage(img.getImage());
        // set the given controller global
        this.controller = controller;
        this.setTitle("N-BackUpper.jar Utility v1.03");
